...
tar archives, and bare Mach-O executables cannot be stapled; Gatekeeper looks
up the CLI ticket online using the signed binary's code identity. The CI job
therefore submits a ZIP containing the standalone CLI, waits for `Accepted`,
and puts those exact signed bytes in both the Homebrew ZIP and CLI `.tar.xz`.
All Cargo builds happen before `codesign`: rebuilding a package target after
signing can replace a file under `target/release` and invalidate its existing
signature.
...
the ZIP or bare CLI; Apple makes that ticket available through Gatekeeper's
online lookup.
