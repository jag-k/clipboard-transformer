...
fn run_command(mut command: Command, input: &[u8], timeout: Duration) -> Result<ProcessOutput> {
    ...
    thread::scope(|scope| {
        let stdout_thread = scope.spawn(move || read_bounded(stdout, MAX_OUTPUT_BYTES));
        let stderr_thread = scope.spawn(move || read_bounded(stderr, MAX_OUTPUT_BYTES));
        let stdin_thread = stdin.map(|mut stdin| {
            scope.spawn(move || match stdin.write_all(input) {
                Ok(()) => Ok(()),
                Err(error) if error.kind() == ErrorKind::BrokenPipe => Ok(()),
                Err(error) => Err(error).context("write shell stdin"),
            })
        });

        let status: Result<ProcessStatus> = (|| {
            ...
            loop {
                ...
                if Instant::now() >= deadline {
                    terminate_process_tree(&mut child)
                        .context("terminate timed out shell process")?;
                    let _ = child.wait();
                    break Ok(ProcessStatus::TimedOut);
                }
        ...
        let stdin_result = stdin_thread.map_or(Ok(()), |thread| {
            thread
                .join()
                .map_err(|_| anyhow!("shell stdin writer panicked"))?
        });
...
mod tests {
    ...
    fn compiled(run: &str) -> CompiledShell {
        compiled_with_timeout(run, "2s")
    }

    #[cfg(unix)]
    fn compiled_with_timeout(run: &str, timeout: &str) -> CompiledShell {
        CompiledShell::new(
            "test-shell",
            ShellSettings {
                ...
                timeout: Some(TimeoutValue::Text(timeout.to_string())),
                env: BTreeMap::new(),
            },
            100 * 1024 * 1024,
    ...
    fn text_shell_uses_stdin_stdout_and_no_match_exit() {
        ...
        let no_match = compiled("exit 3")
            .run_text_with_paths(&paths, "text", &large_input, None)
            .unwrap();
    ...
    #[cfg(unix)]
    #[test]
    fn stdin_backpressure_does_not_disable_the_timeout() {
        let root = tempfile::tempdir().unwrap();
        let paths = test_paths_for_any_platform(root.path());
        let large_input = "x".repeat(2 * 1024 * 1024);
        let error = compiled_with_timeout("sleep 10", "50ms")
            .run_text_with_paths(&paths, "text", &large_input, None)
            .unwrap_err();
        assert!(format!("{error:#}").contains("shell timed out"));
    }
